...
def get_position_df(spaceranger_path):
    ...
    """
    if os.path.isfile(
        os.path.join(spaceranger_path, "outs/spatial/tissue_positions_list.csv")
    ):
        ...
        position_df = pd.read_csv(
            os.path.join(spaceranger_path, "outs/spatial/tissue_positions.csv"),
            header=0,
            names=["barcode", "in_tissue", "row", "col", "imageY", "imageX"],
        )
...
def get_xy_barcode_dict(spaceranger_path):
    ...
    xy_keys = [
        f"{x}_{y}" for x, y in position_df[["imageX", "imageY"]].itertuples(index=False)
    ]
...
def initialize_sample(rid, project_id, sample_path, token, name):
    ...
    response = requests.post(
        config.CLOUD_FN_URL + "add_sample", headers=headers, json=request_json
    )
...
def add_sample_image(rid, project_id, sample_path, token, name):
    ...
    response = requests.post(
        config.CLOUD_FN_URL
        + f"add_sample_image?rid={rid}&project_id={project_id}&sample={name}&img_extension={img_extension}",
        headers=headers,
        data=binary_data,
    )
    handle_server_response(response)
...
def add_resized_fullres(rid, project_id, fullres_path, token, name):
    ...
    response = requests.post(
        config.CLOUD_FN_URL
        + f"add_sample_image?rid={rid}&project_id={project_id}&sample={name}&img_extension={img_extension}&scale_f={scale_f}",
        headers=headers,
        data=binary_data,
    )
    handle_server_response(response)
...
def add_sample_patches(rid, project_id, fullres_path, token, name):
    ...
    assert fullres_extension.upper() in [
        "PNG",
        "JPG",
        "JPEG",
    ], "The fullres image extension must be .PNG, .JPEG, or .JPG"
    ...
    with PIL.Image.open(fullres_path) as fullres_image:
        ...
        for i in range(num_patches):
            ...
            upper = 0
            if i < num_patches - 1:
                right = i * math.floor(patch_width_decimal) + math.floor(
                    patch_width_decimal
                )
            else:
                right = width
            ...
            assert (
                quality > 50
            ), "Was unable to get fullres image patches to high enough quality, try scaling down the fullres image"
            ...
            response = requests.post(
                config.CLOUD_FN_URL
                + f"add_sample_patch?rid={rid}&project_id={project_id}&sample={name}&img_extension={fullres_extension}&current_patch={i+1}&num_patches={num_patches}",
                headers=headers,
                data=binary_data,
            )
            handle_server_response(response)
...
def add_sample_parquet(
    rid, project_id, sample_path, token, genes, is_ensembl, name, qc=False
):
    """
    ...
    if os.path.exists(os.path.join(outs_path, "custom_features.h5ad")):
        ...
        adata = sc.read_10x_h5(
            os.path.join(outs_path, "filtered_feature_bc_matrix.h5"), gex_only=False
        )
    ...
    # get x_y ids
    ids = [
        f"{x}_{y}" for x, y in adata.obs[["imageX", "imageY"]].itertuples(index=False)
    ]
    ...
    response = requests.post(
        config.CLOUD_FN_URL
        + f"add_sample_parquet?rid={rid}&project_id={project_id}&sample={name}",
        headers=headers,
        data=binary_data,
    )
    handle_server_response(response)
...
def add_sample_annotation(rid, project_id, name, token, annotation_path):
    ...
    response = requests.post(
        config.CLOUD_FN_URL
        + f"add_sample_annotation?rid={rid}&project_id={project_id}&sample={name}",
        headers=headers,
        data=binary_data,
    )
...
def add_sample(
    ...
    qc=False,
    fullres_path=None,
    resize_fullres=False,
    upload_fullres=False,
    ...
    if upload_fullres:
        print(
            f"Uploading fullres image from {fullres_path} as multiple patches (backend will stitch them back together)..."
        )
    ...
    if len(genes) == 0:
        ...
        print(
            f"Processing and uploading parquet file (gene expression) from {sample_path}..."
        )
        add_sample_parquet(
            rid, project_id, sample_path, token, genes, is_ensembl, name, qc=qc
        )
...
def main():
    ...
    parser.add_argument(
        help="Paths to Spaceranger objects", dest="sample_paths", nargs="+"
    )
    parser.add_argument(
        help="Path to file with names of genes on each line which will be used to to construct gene expression matrix",
        dest="gene_path",
    )
    parser.add_argument(
        "-n",
        "--names",
        help="Optional alternative names of samples - default names are the Spaceranger subdirectory names",
        dest="sample_names",
        nargs="+",
    )
    parser.add_argument(
        "-e",
        "--ensembl",
        help="Names of genes in gene_path are ensembl codes",
        dest="is_ensembl",
        action="store_true",
    )
    parser.add_argument(
        "-q",
        "--qc_metrics",
        help="Calculate spot QC metrics for the samples and add to the expression profile.",
        dest="qc",
        action="store_true",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-f",
        "--fullres_paths",
        help="Path to the fullres images.",
        dest="fullres_paths",
        nargs="+",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-c",
        "--resize_fullres",
        help=f"Upload a resized version of the fullres image which doesn't exceed {config.MAX_IMG_SIZE}.",
        dest="resize_fullres",
        action="store_true",
    )
    parser.add_argument(
    ...
    sample_names = (
        [None] * len(sample_paths) if args.sample_names is None else args.sample_names
    )
    token = args.token
    ...
    qc = args.qc
    fullres_paths = (
        [None] * len(sample_paths) if args.fullres_paths is None else args.fullres_paths
    )  # default list of None
    resize_fullres = args.resize_fullres
    upload_fullres = args.upload_fullres
    annotation_paths = (
        [None] * len(sample_paths)
        if args.annotation_paths is None
        else args.annotation_paths
    )  # default list of None

    assert sample_names is None or len(sample_names) == len(
        sample_paths
    ), f"The number of arguments in --names should be the the same as the number of Spaceranger paths specified ({len(sample_paths)})"
    assert fullres_paths is None or len(fullres_paths) == len(
        sample_paths
    ), f"The number of arguments in --fullres_paths should be the the same as the number of Spaceranger paths specified ({len(sample_paths)})"
    assert annotation_paths is None or len(annotation_paths) == len(
        sample_paths
    ), f"The number of arguments in --annotations_paths should be the the same as the number of Spaceranger paths specified ({len(sample_paths)})"

    if resize_fullres or upload_fullres:
        assert (
            None not in fullres_paths
        ), f"fullres_paths must be provided if the resize_fullres or upload_fullres flags are present"

    genes = []
    with open(gene_path, "r") as f:
        genes = [l.strip() for l in f.readlines()]

    for sample_path, sample_name, fullres_path, annotation_path in zip(
        sample_paths, sample_names, fullres_paths, annotation_paths
    ):
        add_sample(
            rid,
            project_id,
            ...
            resize_fullres=resize_fullres,
            upload_fullres=upload_fullres,
